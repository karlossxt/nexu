...
module.exports = async (req, res) => {
  ...
  if (req.query && req.query.check === '1') {
    ...
    return res.status(key ? 200 : 503).json({ available: Boolean(key) });
  }
  ...
  try {
    const url = `https://api.tomtom.com/traffic/map/4/tile/flow/relative0/${z}/${x}/${y}.png?tileSize=256&key=${encodeURIComponent(key)}`;
    const upstream = await fetch(url, { headers: { Accept: 'image/png' } });
